    ...
    with torch.no_grad():
        with model.ema_scope():
            ...
            for n in trange(niters, desc="Sampling"):
                ...
                samples, _ = model.sample(cond=semantic_c, struct_cond=init_latent, batch_size=init_image.size(0), timesteps=opt.ddpm_steps, time_replace=opt.ddpm_steps, x_T=x_T, return_intermediates=True, interfea_path=None,
                                        reference_sr=ref_latent, reference_lr=0.5, reference_step=10, reference_range=[10, 1000])
